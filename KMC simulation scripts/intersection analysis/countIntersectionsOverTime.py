...
def faceFaceIntersect(face1, face2, V):
    ...
    for edge in face1Edges:
        if edgeFaceIntersect(edge, face2, V):
            return True
    for edge in face2Edges:
        if edgeFaceIntersect(edge, face1, V):
            return True
    return False
    ...
    for folderName in dirList:
        ...
        for VfileName in fileList:
            if "coordinates" in VfileName:
                ...
                F = np.loadtxt(os.path.join(F_location, folderName, faceFilename), delimiter=",", dtype=int)
        ...
        if len(F) > 96:
            countData.pop(simName)
            print(f"popped {folderName} because it was overgrown (N_total={len(F)}).")
    return countData

def main():
    V_location = r'C:\Users\mason\OneDrive\Desktop\code\openMesh\test_20250924_toroid\off_target\data_4221_toroid_R_exc0_1_off_target_coordinates' # vertex coordinates file
    F_location = r'C:\Users\mason\OneDrive\Desktop\code\openMesh\test_20250924_toroid\off_target\data_4221_toroid_R_exc0_1_off_target_faces' # faces

    outputLocation = r'C:\Users\mason\OneDrive\Desktop\code\openMesh\analysis\intersectionAnalysis\intersectionData'
    outputName = "intersections_4221_toroid_R_exc0_1_off_target.json"
